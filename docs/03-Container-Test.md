# How To Locally Test Containerized NGINX Plus OIDC for Okta Integration

Take the following steps to run NGINX Plus in a Docker container and test OIDC for Okta integration.
...
- **Run** a Web Browser with `https://nginx.okta.test`, and enter an `IdP application`'s `client ID`:

  ![](./img/run-browser.png)

- **Login** to click `Sign in` button:

  Enter your name and password which are registered in Okta.
